...
def generate(prompt: str, uncond_prompt: str, input_image=None, strength=0.8, do_cfg=True, cfg_scale=7.5, 
             ...
             device=None, idle_device=None, tokenizer=None): # unconditional prompt, do_cfg = do classifier free guidance
    with torch.no_grad():
        if not (0 < strength <= 1):
            raise ValueError("strength numst be between 0 and 1")
        ...
        if do_cfg:
            ...
            uncond_tokens = tokenizer.batch_encode_plus([uncond_prompt], padding="max_length", max_length=77).input.ids
        ...
        for i, timestep in enumerate(timesteps):
            ...
            if do_cfg:
                # (Batch_Size, 4, Lantes_Height, Latents_Width) -> (2 * Batch_Size, 4, Latents_Height, Latents_Width)
        ...
        # (Batch_Size, Channel, Height, Width) -> (Batch_Siz, Height, Width, Channel)
        images = images.primute(0,2,3,1)
